...
def decodeData(message):
	...
	if ( canid == '300' ):
		val = float(int(data[12:13],8)*256+int(data[15:16],8))
		Map = val/1000
		val = int(data[0:1],8)*256+int(data[3:4],8)
		Rpm = val
		val = int(data[6:7],8)*100/255
		Tps = val
	if ( canid == '301' ):
		Lambda2 = float(int(data[6:7],8))*2*14.7/255
	if ( canid == '305' ):
		Lambda1=  float(int(data[0:1],8))*2*14.7/255
	if ( canid == '306' ):
		Fuel = float(int(data[18:19],8)*256+int(data[21:22],8))*0.1
	if ( canid == '308'):
		val = 0.0
		val = float(int(data[0:1],8)*256+int(data[3:4],8))
		Volt = val
		Volt = Volt*18/1023
	if ( canid == '30b'):
		Temp = float((int(data[0:1],8)))*150/255-10
		Air = float((int(data[9:10],8)))*150/255-10
...
class MainWindow(QMainWindow):
    ...
    def write_to_csv(self, filename, data_arrays, headers):
        with open(filename, 'w', newline='') as csvfile:
            csv_writer = csv.writer(csvfile)
            csv_writer.writerow(headers)
            csv_writer.writerows(zip(*data_arrays))
